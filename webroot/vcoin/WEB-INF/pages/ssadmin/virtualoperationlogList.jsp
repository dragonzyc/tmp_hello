	...
	<div class="panelBar">
		<ul class="toolBar">
			...
			<shiro:hasPermission name="ssadmin/auditVirtualOperationLog.html">
				...
				<li><a class="edit"
					href="ssadmin/goVirtualOperationLogJSP.html?uid={sid_user}&url=ssadmin/sendVirtualOperationLog"
					height="320" width="800" target="dialog"
					rel="sendVirtualOperationLog"><span>放发冻结</span>
				</a>
				</li>	
			</shiro:hasPermission>
		</ul>
	...
	<table class="table" width="100%" layoutH="138">
		<thead>
			<tr>
				...
				<th width="60" orderField="ffrozenQty"
					<c:if test='${param.orderField == "ffrozenQty" }'> class="${param.orderDirection}"  </c:if>>冻结数量</th>
				<th width="60" orderField="fqty"
					<c:if test='${param.orderField == "fqty" }'> class="${param.orderDirection}"  </c:if>>已发放数量</th>	
		...
		<tbody>
			<c:forEach items="${virtualoperationlogList}"
				...
				<tr target="sid_user" rel="${virtualoperationlog.fid}">
				    <td>${virtualoperationlog.fisSendMsg}</td>
					...
					<td><fmt:formatNumber value="${virtualoperationlog.ffrozenQty}" pattern="##.######" maxIntegerDigits="15" maxFractionDigits="4"/></td>
					<td><fmt:formatNumber value="${virtualoperationlog.fqty}" pattern="##.######" maxIntegerDigits="15" maxFractionDigits="4"/></td>
