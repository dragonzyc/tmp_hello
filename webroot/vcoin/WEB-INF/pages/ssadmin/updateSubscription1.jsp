	...
	<form method="post" action="ssadmin/updateSubscription1.html"
		...
		<div class="pageFormContent nowrap" layoutH="97">
				...
				<dd>
					<select
						...
						<c:forEach items="${coins}" var="type">
							<c:if test="${type.fid == (subscription.fvirtualcointypeCost == null?0:subscription.fvirtualcointypeCost.fid)}">
			...
			<dl>
				<dt>支付比例1：</dt>
				<dd>
					<input type="text" name="fcoinRate1" class="required number"  value="${subscription.fcoinRate1}"/>
					<span>小数,不用填0</span>
				</dd>
			</dl>
			<dl>
				<dt>币的价格1：</dt>
				<dd>
					<input type="text" name="fcoinPrice1" class="required number"  value="${subscription.fcoinPrice1}"/>
					<span>人民币填1,虚拟币按价格填价格，按数量填0</span>
				</dd>
			</dl>
			<dl>
				<dt>币1是否奖励：</dt>
				<dd>
					<c:choose>
						<c:when test="${subscription.fiscoin1}">
							<input type="checkbox" name="fiscoin1" checked='1' />
						</c:when>
						<c:otherwise>
							<input type="checkbox" name="fiscoin1" />
						</c:otherwise>
					</c:choose>
				</dd>
			</dl>
			<dt>支付类型2：</dt>
				<dd>
					<select
						...
						<c:forEach items="${coins}" var="type">
							<c:if test="${type.fid == (subscription.fvirtualcointypeCost2 == null?0:subscription.fvirtualcointypeCost2.fid)}">
								<option value="${type.fid}" selected="true">${type.fname}</option>
							</c:if>
							<c:if test="${type.fid != (subscription.fvirtualcointypeCost2 == null?0:subscription.fvirtualcointypeCost2.fid)}">
								<option value="${type.fid}">${type.fname}</option>
			...
			<dl>
				<dt>支付比例2：</dt>
				<dd>
					<input type="text" name="fcoinRate2" class="required number"  value="${subscription.fcoinRate2}"/>
					<span>小数,不用填0</span>
				</dd>
			</dl>
			<dl>
				<dt>币的价格2：</dt>
				<dd>
					<input type="text" name="fcoinPrice2" class="required number"  value="${subscription.fcoinPrice2}"/>
					<span>人民币填1,虚拟币按价格填价格，按数量填0</span>
				</dd>
			</dl>
			<dl>
				...
				<dd>
					<c:choose>
						<c:when test="${subscription.fiscoin2}">
							<input type="checkbox" name="fiscoin2" checked='1' />
						</c:when>
						<c:otherwise>
							<input type="checkbox" name="fiscoin2" />
						</c:otherwise>
					</c:choose>
				</dd>
			</dl>
