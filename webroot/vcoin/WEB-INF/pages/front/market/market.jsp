			...
			<div class="col-xs-12 rightarea market">
				<ul id="marketheader" class="nav nav-tabs rightarea-tabs">
					...
					<c:forEach items="${ftrademappings }" var="v">
						<c:if test="${v.fvirtualcointypeByFvirtualcointype1.fname=='金贝' && v.fvirtualcointypeByFvirtualcointype2.fname !='智能本票'}">
						<li class="${v.fid==symbol?'active':'' }">
						    <c:choose>
						    <c:when test="${v.fid==symbol }">
						    <a href="javascript:void(0)">${v.fvirtualcointypeByFvirtualcointype2.fname }-${v.fvirtualcointypeByFvirtualcointype1.fname }</a>
						    </c:when>
						    <c:otherwise>
						    <a href="/market.html?symbol=${v.fid }">${v.fvirtualcointypeByFvirtualcointype2.fname }-${v.fvirtualcointypeByFvirtualcointype1.fname }</a>
						    </c:otherwise>
						    </c:choose>
						</li>
						</c:if>
					...
					<c:forEach items="${ftrademappings }" var="v">
						<c:if test="${v.fvirtualcointypeByFvirtualcointype1.fname=='金海贝'}">
						<li class="${v.fid==symbol?'active':'' }">
						    <c:choose>
						    <c:when test="${v.fid==symbol }">
						    <a href="javascript:void(0)">${v.fvirtualcointypeByFvirtualcointype2.fname }-${v.fvirtualcointypeByFvirtualcointype1.fname }</a>
					...
					<%-- <li style="width: 170px;float: right;padding-top: 10px;">
						<div>
					  		法币类型:
						<select style="height: 25px;width: 100px;" id="coinType">
						<c:forEach items="${requestScope.constant['fbs']}" var="v">
						<c:if test="${type == v.fid }">
